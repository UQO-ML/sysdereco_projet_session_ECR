...
class MultiOutput(ModelOutput):
    ...
    emo_logits: Optional[torch.FloatTensor] = None
...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        emo_labels=None,
        loss_function = None,
        label_weight_batch = None,
        weighted_loss = False,
        rec_sentiment_loss = None,
        use_sentiment = False,
        copy_logit = None,
        copy_w = None
    ):
        ...
        if emo:
            ...
            emo_logits = hidden_states[range(batch_size), sequence_lengths]
            emo_logits @= emo_embeds.T  # (bs, n_item)

            if emo_labels is not None:
                if loss_function is not None:
                    emo_loss = loss_function(emo_logits, emo_labels)
                # loss_fct = CrossEntropyLoss()
                else:
                    emo_loss = F.cross_entropy(emo_logits, emo_labels)
        ...
        if rec:
            ...
            rec_logits = hidden_states[range(batch_size), sequence_lengths]
            rec_logits @= entity_embeds.T  # (bs, n_item)
            if copy_logit is not None:
                rec_logits = rec_logits + copy_w * copy_logit
            if use_sentiment == True:
                loss_weight = rec_sentiment_loss
            else:
                loss_weight = label_weight_batch

            if rec_labels is not None and loss_weight is not None and weighted_loss == True:
                # loss_fct = CrossEntropyLoss()
                rec_loss = F.cross_entropy(rec_logits, rec_labels, reduction='none')
                rec_loss = torch.einsum('l,l->l', rec_loss, loss_weight)
                rec_loss = torch.mean(rec_loss)
            else:
                rec_loss = F.cross_entropy(rec_logits + 1e-10, rec_labels)
